...
class TestInitDatabase:
    ...
    def test_writes_schema_version_to_config(self, tmp_path):
        ...
        with patch("nber_cli.db.db.Path.home", return_value=home):
            db.init_database(db_path)
...
class TestConnectionBoundary:
    def test_record_query_checks_schema_and_writes_with_one_engine(self, tmp_path):
        ...
        with patch("nber_cli.db.db.create_engine", side_effect=tracked_create_engine):
...
class TestMigrateDatabase:
    ...
    def test_rejects_future_schema_without_moving_database(self, tmp_path):
        ...
        with (
            patch("nber_cli.db.db.Path.home", return_value=home),
            patch("nber_cli.db.db.get_database_path", return_value=old),
...
class TestDatabasePathSecurity:
    def test_init_rejects_path_outside_home_on_unix(self, tmp_path):
        ...
        with patch("nber_cli.db.db.sys.platform", "linux"):
    ...
    def test_init_allows_path_within_symlinked_home_on_unix(self, tmp_path):
        ...
        with (
            patch("nber_cli.db.db.sys.platform", "linux"),
            patch("nber_cli.db.db.Path.home", return_value=linked_home),
        ):
            db.init_database(inside_path)
    ...
    def test_init_allows_any_path_on_windows(self, tmp_path):
        outside_path = tmp_path.parent / "outside.db"
        with patch("nber_cli.db.db.sys.platform", "win32"):
            db.init_database(outside_path)
