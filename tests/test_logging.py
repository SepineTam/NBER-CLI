...
class TestConfigureLogging:
    def test_default_level_is_warning(self, tmp_path):
        from nber_cli.logging_config import configure_logging

        with patch("nber_cli.logging_config._default_log_dir", return_value=tmp_path):
...
class TestFetcherLogging:
    def test_load_text_sync_logs_request(self, tmp_path, caplog):
        from nber_cli.fetcher import _load_text_sync
        ...
        with patch("nber_cli.fetcher.urlopen", return_value=FakeResponse()):
            with caplog.at_level(logging.DEBUG, logger="nber_cli.fetcher"):
...
"""
        ...
        with patch("nber_cli.feed._load_text_sync", return_value=sample_xml):
            with patch("nber_cli.feed.db.get_database_path", return_value=tmp_path / "feed.db"):
                with caplog.at_level(logging.DEBUG, logger="nber_cli.feed"):
