...
from nber_cli.core.models import DownloadBatchResult, DownloadFailure
from nber_cli.fetch import (
    download_multiple_papers,
    download_paper,
    download_paper_to_file,
)
from nber_cli.fetch.download import _create_connector
...
class TestDownloadPaperToFile:
    ...
    async def test_successful_download(self, tmp_path):
        ...
        with patch("nber_cli.fetch.download.ClientSession", return_value=fake_session):
            with patch("nber_cli.fetch.download._retry_async", side_effect=_no_retry):
                with patch("nber_cli.fetch.download._USER_AGENT") as mock_ua:
    ...
    async def test_user_agent_header(self, tmp_path):
        ...
        with patch("nber_cli.fetch.download.ClientSession", return_value=fake_session) as mock_client_session:
            with patch("nber_cli.fetch.download._retry_async", side_effect=_no_retry):
                with patch("nber_cli.fetch.download._USER_AGENT") as mock_ua:
...
class TestDownloadPaper:
    ...
    async def test_download_paper_uses_correct_filename(self, tmp_path):
        with patch("nber_cli.fetch.download.download_paper_to_file") as mock_download:
...
class TestDownloadMultiplePapers:
    ...
    async def test_rejects_invalid_concurrency_immediately(self, tmp_path, concurrency):
        with patch("nber_cli.fetch.download.ClientSession") as mock_session:
    ...
    async def test_invalid_file_concurrency_falls_back_without_hanging(
        ...
        with (
            patch("nber_cli.fetch.download.NBER_CLI_CONFIG", runtime_config),
            patch(
                "nber_cli.fetch.download.download_paper",
    ...
    async def test_all_success(self, tmp_path):
        with patch("nber_cli.fetch.download.download_paper") as mock_download:
    ...
    async def test_concurrent_execution(self, tmp_path):
        ...
        with patch("nber_cli.fetch.download.download_paper", side_effect=slow_download):
...
class TestPathValidation:
    def test_validate_paper_id_accepts_w_prefix(self):
        from nber_cli.fetch.download import _validate_paper_id
    ...
    def test_validate_download_path_rejects_outside_cwd(self):
        from nber_cli.fetch.download import _validate_download_path
