...
| `desktop.server_port` | 是 | `~/.nber-cli/config.json` 或 Desktop 设置页；重启 Desktop 后生效 |
| `desktop.feed_refresh_interval_minutes` | 是 | `~/.nber-cli/config.json` 或 Desktop 设置页；需要跨 Desktop 重启保留时使用 `1`–`65535` |
...
`desktop.server_port` 指定本地 HTTP sidecar 使用的 loopback 端口，范围为 `1024` 至 `65535`；修改后需要重启 Desktop 才会生效。Python 设置 API 接受任意正整数 `desktop.feed_refresh_interval_minutes`，但 Rust 外壳只能保留不超过 `65535` 的值；保存更大值后，下次启动 Desktop 会回退到 `60`。在两端契约统一前请使用 `1`–`65535`。
