...
| `desktop.feed_refresh_interval_minutes` | 是 | `~/.nber-cli/config.json` 或 Desktop 设置页 |
...
`desktop.server_port` 指定本地 HTTP sidecar 使用的 loopback 端口，范围为 `1024` 至 `65535`；修改后需要重启 Desktop 才会生效。`desktop.feed_refresh_interval_minutes` 控制 Desktop 自动刷新 feed 的间隔，必须是正整数。
