...
| `desktop.server_port` | 仅兼容旧配置/HTTP API | Desktop 0.9.0 忽略；可选 `nber-server` 仍可使用 |
...
Desktop 0.9.0 不运行本地服务器，因此会忽略旧的 `desktop.server_port`。`desktop.feed_refresh_interval_minutes` 控制 Rust 原生 RSS 自动刷新，范围为 `1`–`65535`。
