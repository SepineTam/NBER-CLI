...
| 本地 HTTP server | `src/nber_server/` | 为 Desktop 和本地集成提供 loopback 健康检查、feed、论文、已读状态和设置接口。 |
| Desktop 应用 | `desktop/` | 由 Tauri 启动内置 Python sidecar，并呈现 React 研究工作台。 |
...
- Desktop HTTP server 绑定 loopback，仅用于本机；如需暴露到主机之外，必须单独进行安全审查。
