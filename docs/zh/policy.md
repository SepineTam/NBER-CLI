...
- **用户机器上的默认本地持久化**：NBER-CLI 会在用户机器上保留一个本地 SQLite 数据库，默认位于 `~/.nber-cli/nber.db`（或 `nber-cli db init` / `db migrate` 配置的路径 / `sqlite:///...` URL），并保留一个用户配置文件 `~/.nber-cli/config.json`。数据库通过 SQLModel/SQLAlchemy 访问，除非用户自行复制或导出，否则它只留在用户机器上。下列命令和工具会在没有额外用户输入的情况下默认写入该数据库：
  ...
  - Desktop 使用 Rust 直接读取配置中的 SQLite 数据库，在数据库为空时自动请求 RSS，并在应用处于活动状态时按设置的间隔刷新。
  - 在 Desktop 中打开论文可能会向 NBER 请求元数据、更新 `info_cache`，并把已读状态写入 `read_status`；手动切换已读或未读也会更新该表。
  - Desktop 会创建或更新 `~/.nber-cli/config.json`，读取共享的 `feed.db-path`，并且不会启动或记录 Python sidecar 进程。
- **本地数据清理**：`feed clean` 和 `info cache clear` 在交互式确认后会删除缓存记录。`query_log`、`download_log`、`info_log`、`read_status` 和 `feed_fetches` 都没有 CLI 清理命令。停止 Desktop、CLI、MCP 与 HTTP server 进程后，可以通过手动 `sqlite3` 维护，或删除/替换数据库来清理。`feed clean --all` 只删除 `feed_items`。
