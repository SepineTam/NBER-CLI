...
- **用户机器上的默认本地持久化**：NBER-CLI 会在用户机器上保留一个本地 SQLite 数据库，默认位于 `~/.nber-cli/nber.db`（或 `nber-cli db init` / `db migrate` 配置的路径 / `sqlite:///...` URL），并保留一个用户配置文件 `~/.nber-cli/config.json`。数据库通过 SQLModel/SQLAlchemy 访问，除非用户自行复制或导出，否则它只留在用户机器上。下列命令和工具会在没有额外用户输入的情况下默认写入该数据库：
  ...
  - Desktop 启动和打开论文只读取本地 SQLite，不会访问 NBER。全新安装在第一次成功刷新 Feed 时创建数据库。
  - 用户手动刷新，或应用已打开且窗口可见时自动刷新间隔到期，会启动内置单次 worker。它向 NBER 请求 RSS 和论文元数据，更新 `feed_items`、`feed_fetches`、`info_cache` 后退出。
  - 在 Desktop 中打开论文会把已读状态写入 `read_status`；手动切换已读或未读也会更新该表。本地标签操作会写入四张 `desktop_*` 表。
  - Desktop 会创建或更新 `~/.nber-cli/config.json`，读取共享的 `feed.db-path`，并且不会运行长期 Python sidecar 或本地 HTTP server。
- **本地数据清理**：`feed clean` 和 `info cache clear` 在交互式确认后会删除缓存记录。`query_log`、`download_log`、`info_log`、`read_status`、`feed_fetches` 和四张 `desktop_*` 表都没有 CLI 批量清理命令。停止 Desktop、CLI、MCP 与 HTTP server 进程后，可以通过可用的逐项 Desktop 操作、手动 `sqlite3` 维护，或删除/替换数据库来清理。`feed clean --all` 只删除 `feed_items`。
...
用户应自行承担其请求、下载、本地存储选择，以及后续使用 NBER 材料所产生的责任。维护者不对访问被拒、账号或网络受限、版权或许可争议、数据丢失、服务中断、政策违反，或因用户使用 NBER-CLI 而产生的其他后果承担责任。
