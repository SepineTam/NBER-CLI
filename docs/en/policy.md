...
- **Default local persistence on the user machine**: NBER-CLI does keep a local SQLite database at `~/.nber-cli/nber.db` (or the path / `sqlite:///...` URL configured by `nber-cli db init` / `db migrate`) and a user config file at `~/.nber-cli/config.json`. The database is accessed through SQLModel/SQLAlchemy and remains on the user's machine unless the user copies or exports it. By default the following commands and tools write to that database without any extra user input:
  ...
  - Desktop uses native Rust code to read the configured SQLite database, fetches RSS when the database is empty, and refreshes it at the configured interval while the app is active.
  ...
  - Desktop creates or updates `~/.nber-cli/config.json`, honors the shared `feed.db-path`, and does not start or log a Python sidecar process.
- **Cleaning up local data**: the `feed clean` and `info cache clear` commands delete cache rows after an interactive confirmation. The `query_log`, `download_log`, `info_log`, `read_status`, and `feed_fetches` tables have no CLI cleanup command. Remove them with manual `sqlite3` maintenance or by deleting/replacing the database after stopping Desktop, CLI, MCP, and HTTP server processes. `feed clean --all` deletes `feed_items` only.
