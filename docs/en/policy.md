...
- **Default local persistence on the user machine**: NBER-CLI does keep a local SQLite database at `~/.nber-cli/nber.db` (or the path / `sqlite:///...` URL configured by `nber-cli db init` / `db migrate`) and a user config file at `~/.nber-cli/config.json`. The database is accessed through SQLModel/SQLAlchemy and remains on the user's machine unless the user copies or exports it. By default the following commands and tools write to that database without any extra user input:
  ...
  - Desktop startup and paper opening read local SQLite data and do not contact NBER. A new installation creates the database during its first successful Feed refresh.
  - A manual Feed refresh, or an automatic refresh whose interval elapses while the app is open and visible, starts the bundled one-shot worker. That worker requests RSS and paper metadata from NBER, updates `feed_items`, `feed_fetches`, and `info_cache`, and then exits.
  - Opening a paper in Desktop writes its read state to `read_status`; manual read/unread changes update the same table. Local tag actions write the four `desktop_*` tables.
  - Desktop creates or updates `~/.nber-cli/config.json`, honors the shared `feed.db-path`, and does not run a long-lived Python sidecar or local HTTP server.
- **Cleaning up local data**: the `feed clean` and `info cache clear` commands delete cache rows after an interactive confirmation. The `query_log`, `download_log`, `info_log`, `read_status`, `feed_fetches`, and four `desktop_*` tables have no CLI bulk-cleanup command. Remove them with supported individual Desktop actions where available, manual `sqlite3` maintenance, or by deleting/replacing the database after stopping Desktop, CLI, MCP, and HTTP server processes. `feed clean --all` deletes `feed_items` only.
